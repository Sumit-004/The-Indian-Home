...
const BestSeller = () => {
    ...
    const { products, addToCart, backendUrl, category } = useContext(dataContext);
    ...
    const filteredProducts = products.filter(product => {
        const matchesCategory = product.category === 'bestseller';
        ...
        <>
            <div className='mt-24 flex items-center justify-center gap-2'>
                <div className='flex items-center ml-10 justify-center gap-2'>
                    <p className='prata-regular md:text-4xl text-3xl text-black font-semibold'><span className='text-gray-700'>Best</span> Seller</p>
                    <p className='w-8 h-[2px] bg-black'></p>
                </div>
            </div>            <div className='flex flex-wrap justify-center w-full md:mx-4 mx-1'>
                {filteredProducts.map((card, index) => (
                    <div className='md:my-6 my-5' key={index} id={card._id}>
                        <div className=' md:h-[340px] md:w-[290px] h-[185px] w-[140px] md:p-2 p-0 flex flex-col md:mx-6 mx-2.5'>
                            ...
                            <div className='md:h-[23%] h-[35%] md:text-[15px] text-[9px]'>
                                ...
                                <div className='md:flex md:flex-row flex-col justify-between h-0.5/2 md:text-[16px] text-[10px]'>
                                    <div>
                                        <span className='line-through'>Rs. 459/-</span>
                                        <span className='font-semibold'>Rs. {card.price}/-</span>
                                    </div>
                                    <div className='flex justify-center mt-2'>
                                        <button onClick={() => addToCart(card._id)} className='bg-blue-900 md:text-[14px] text-[10px] px-2 py-1 text-white rounded-sm hover:bg-blue-700 transition-all duration-200 cursor-pointer'>Add+</button>
                                    </div>
                                </div>
                            </div>
                        </div>
